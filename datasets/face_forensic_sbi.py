...
class SBIFaceForensic(MasterDataset):
    ...
    @staticmethod
    def _get_generated_image_path(path, type, original_type="aligned_Original_81"):
        path = path.replace(original_type, f"{original_type}_{type}")
        path = path.replace(".png", f".{type}.jpg")
        return path
    ...
    def __getitem__(self, idx):
        ...
        while flag:
            try:
                ...
                generated_images = {
                    "sd":  (lambda *_, **__: {"image": self._load_img(SBIFaceForensic._get_generated_image_path(img_path, "sd" ))})  if self.split in {"train", "val"} else None,
                    "gan": (lambda *_, **__: {"image": self._load_img(SBIFaceForensic._get_generated_image_path(img_path, "gan"))})  if self.split in {"train", "val"} else None,
                }
                if self.split == 'test':
                ...
                if not self.dynamic_fxray:
                    ...
                    if self.train:
                        ...
                        if not f_lms.any():
                            raise MissingLandmarkError('Can not find fake copy image of empty landmarks!')
                        ...
                        if self.split == 'train':
                            if np.random.rand() < 0.5:
                                generated_images = {
                                    k: (lambda *_, **__: {"image": sbi_hflip(v()["image"], None, f_lms, None)[0]}) 
                                    if v is not None 
                                    else None 
                        ...
                        img_f, mask_f, img, mask = gen_target(
                            img, f_lms, margin=[margin, margin], 
                            pmm=self._cfg.PMM, 
                            generated_image_data = generated_images)
                target = None
                target_f = None
        ...
        if self.train:
            if not self.return_vid_id:
                ...
                return patch_img_trans_f, patch_heatmap_f, patch_target_f, patch_cstency_f, patch_img_trans, patch_heatmap_r, patch_target_r, patch_cstency_r, vid_id
        else:
