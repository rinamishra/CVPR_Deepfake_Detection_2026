...
ROOT = '/data/LAA-dataset'
SAVE_DIR = f'{ROOT}/FaceForensics++'
...
def facecrop(model, org_path, save_path, period=1, num_frames=10, dataset='original', label=None, mask_path=None, padding=PADDING):
    ...
    for cnt_frame in range(frame_count_org):
        ...
        try:
            ...
            try:
                ...
                for face_idx in range(len(faces)):
                    if not len(faces[face_idx]['bbox']) == 4:
                        tqdm.write('No bbox in {}:{}'.format(cnt_frame,os.path.basename(org_path)))
                        continue
        ...
        if face_crop is not None:
            ...
            print(f"failed to save {cnt_frame}:{os.path.basename(org_path)}, no face found")
...
if __name__=='__main__':
    ...
    if args.dataset in ['Original','DeepFakeDetection','FaceShifter','Face2Face','Deepfakes','FaceSwap','NeuralTextures']:
        ...
        with open(f'{dataset_path}List_of_testing_videos.txt') as f:
