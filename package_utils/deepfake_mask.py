...
class randomhull(Mask):  # pylint: disable=invalid-name
    """ Random subset of landmark points for the mask """
    def build_mask(self):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)
        landmarks = np.array(self.landmarks).reshape((-1, 2))
        total_num_landmarks = landmarks.shape[0]
        chosen_num_landmarks = random.randint(3, total_num_landmarks-1)
        landmarks = landmarks[np.random.permutation(total_num_landmarks)[:chosen_num_landmarks]]
        hull = cv2.convexHull(  # pylint: disable=no-member
            landmarks)
        cv2.fillConvexPoly(mask, hull, 255.0, lineType=cv2.LINE_AA)  # pylint: disable=no-member
        return mask
    
class noisyFewPointNotTooSmallMask(Mask):  # pylint: disable=invalid-name
    """ mask based on few points, but check to not be too small, also jitter the points a bit """
    def build_mask(self):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)
        all_landmarks = np.array(self.landmarks).reshape((-1, 2))
        total_num_landmarks = all_landmarks.shape[0]
        chosen_num_landmarks = random.randint(3, 5)
        area = 0
        image_area = self.face.shape[0] * self.face.shape[1]
        while area / image_area < 0.07:
            landmarks = all_landmarks[np.random.permutation(total_num_landmarks)[:chosen_num_landmarks]]
            hull = cv2.convexHull(  # pylint: disable=no-member
                landmarks)
            hull = hull + np.random.randn(*hull.shape) * 5
            hull = hull.astype(int)
            area = poly_area(hull)
        cv2.fillConvexPoly(mask, hull, 255.0, lineType=cv2.LINE_AA)  # pylint: disable=no-member
        return mask
    
class singleFacepartMask(components):
    def build_mask(self):
        mask = np.zeros(self.face.shape[0:2] + (1, ), dtype=np.float32)

        parts = self.get_parts()

        part = parts[np.random.choice(len(parts))]
...
def poly_area(edges):
    ...
    determinants = x * shifted_y - shifted_x * y
    two_A = np.abs(determinants.sum())
    return two_A / 2

def random_get_hull(landmark,img1, cfg):
    mask_types = [0,1,2,3]
    if cfg.small_masks:
        mask_types.append(4)
    if cfg.binomial_masks:
        mask_types.append(5)
    if cfg.facepart_masks:
        mask_types.append(6)
    hull_type = random.choice(mask_types)
    if hull_type == 0:
        ...
        mask = noisyFewPointNotTooSmallMask(landmarks=landmark.astype('int32'),face=img1, channels=3).mask
    elif hull_type == 5:
        mask = randomhull(landmarks=landmark.astype('int32'),face=img1, channels=3).mask
    elif hull_type == 6:
        mask = singleFacepartMask(landmarks=landmark.astype('int32'),face=img1, channels=3).mask
    return mask/255


def dynamic_blend(source,target,mask, cfg):
    ...
    r = random.random()
    h, w, _ = source.shape
    if r < cfg.poisson_prob:
        mask_blured_ = (mask_blured_*255).astype(np.uint8)
        
        mask_ceil = np.where(mask_blured_[:, :, 0] > 0, 1, 0)
        along_w = mask_ceil.max(axis=0)
        along_h = mask_ceil.max(axis=1)
        w1 = np.argmax(along_w)
        w2 = w - np.argmax(along_w[::-1])
        h1 = np.argmax(along_h)
        h2 = h - np.argmax(along_h[::-1])

        w_center = (w1 + w2) // 2
        h_center = (h1 + h2) // 2

        img_blended = cv2.seamlessClone(source, target, mask_blured_, (w_center, h_center), cv2.NORMAL_CLONE)
    else:
