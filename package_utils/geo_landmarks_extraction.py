...
class LandmarkUtility(object):
    ...
    def build_data(self, img_paths, file_names, **kwargs):
        ...
        for i, (p, f) in enumerate(zip(img_paths, file_names)):
            fake_type = p.split('/')[-3] if self.fake_types != ['original'] else self.fake_types[0]
            ...
            data["label"] = int(any([x in p for x in {"method", "1", "synthesis"}]))
        return data
...
if __name__ == '__main__':
    ...
    if extract_landmark:
        ...
        if save_aligned:
            for ft in lm_ins.fake_types:
                os.makedirs(f'{lm_ins.image_root}{lm_ins.split}/{lm_ins.data_type}/aligned_{ft}_{cfg.PREPROCESSING.N_LANDMARKS}', exist_ok=True)
            for i, img_p in enumerate(tqdm(img_paths, dynamic_ncols=True)):
                rot_img = rot_imgs[i]
                fn = file_names[i]
                vid_id = img_p.split('/')[-2]
                fake_type = img_p.split('/')[-3]
                os.makedirs(f'{lm_ins.image_root}{lm_ins.split}/{lm_ins.data_type}/aligned_{fake_type}_{cfg.PREPROCESSING.N_LANDMARKS}/{vid_id}', exist_ok=True)

                if lm_ins.fake_types[0]:
                    aligned_img_p = img_p.replace(lm_ins.fake_types[0], f'aligned_{fake_type}_{cfg.PREPROCESSING.N_LANDMARKS}')
                else:
                    raise RuntimeError()
                    aligned_img_p = img_p.replace("frames", f'frames/aligned_{lm_ins.fake_types[0]}_{cfg.PREPROCESSING.N_LANDMARKS}')
