...
from datasets.sbi_test_wrapper import SBITestWrapper
import logging
...
def main(args = None):
    ...
    if cfg.MODEL.type == "EFN":
        ...
        model = Xception_simple(num_classes=2)
        ...
        start_epoch = 0
    elif cfg.MODEL.type == "SPSL":
        from models.networks.spsl import SPSL
        model = SPSL(num_classes=1, shallow=cfg.MODEL.shallow)
        logger.info(f"loaded model SPSL")
        start_epoch = 0
        ...
        try:
            model.load_state_dict(torch.load(cfg.TEST.pretrained)["state_dict"])
        except Exception as e:
            print(e)
            model.net.load_state_dict(
                {k[4:]: v for k, v in torch.load(cfg.TEST.pretrained)["model"].items() if k.startswith("net.")}
            )
        logger.info(f"loaded model SBI")
    else:
        model = build_model(cfg.MODEL, MODELS).to(torch.float64)
    logger.info(f"Degradation settings: {cfg.DATASET.PMM.DEGRADATIONS}")
    ...
    if image is not None and task == 'test_img':
        ...
        if cfg.DATASET.PMM.DEGRADATIONS.degradations_p != 0:
            ...
            degrader = blindsr.PDM_DatasetWrapper(None, cfg.DATASET.PMM.DEGRADATIONS)
            input = degrader.degradation_fn(input)
        with torch.no_grad():
            st = time.time()
    ...
    if task == 'eval':
        ...
        if cfg.DATASET.DATA.TEST.NAME == "SBI":
            test_dataset = SBITestWrapper(test_dataset)
        if cfg.DATASET.PMM.DEGRADATIONS.degradations_p != 0:
            from lib.pdm import utils_blindsr as blindsr
            test_dataset = blindsr.PDM_DatasetWrapper(test_dataset, cfg.DATASET.PMM.DEGRADATIONS)
        ...
        with torch.no_grad():
            ...
            # results = torch.stack([total_labels.squeeze(), total_preds.squeeze()], dim=0).T
            # logger.info(results)
            # logger.info(torch.stack(sorted(results, key=lambda x: x[1])))
