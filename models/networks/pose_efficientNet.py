...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, model_name=None):
        super().__init__()
        model_name = model_name or "efficientnet-b4"
        ...
        if self._global_params.include_hm_decoder:
            ...
            hm_config = {
                "efficientnet-b4": {
                    "decoder_filters": [1792, 448, 160, 56],
                    "inception_block_c3": 112,
                    "inception_block_not_c3": 56
                },
                "efficientnet-b5": {
                    "decoder_filters": [2048, 512, 176, 64],
                    "inception_block_c3": 128,
                    "inception_block_not_c3": 64
                } 
            }
            this_hm_config = hm_config[model_name]
            
            self.hm_decoder_filters = this_hm_config["decoder_filters"]
            print(self.hm_decoder_filters)
            ...
            if self._global_params.use_c3:
                #self.inception_block = InceptionBlock(128, 128, stride=1, pool_size=3)
                self.inception_block = InceptionBlock(this_hm_config["inception_block_c3"], this_hm_config["inception_block_c3"], stride=1, pool_size=3)
            else:
                #self.inception_block = InceptionBlock(64, 64, stride=1, pool_size=3)
                self.inception_block = InceptionBlock(this_hm_config["inception_block_not_c3"], this_hm_config["inception_block_not_c3"], stride=1, pool_size=3)
...
class PoseEfficientNet(EfficientNet):
    def __init__(self, model_name, in_channels=3, **override_params):
        ...
        blocks_args, global_params = get_model_params(model_name, override_params)
        super().__init__(blocks_args, global_params, model_name=model_name)
