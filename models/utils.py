...
def load_pretrained(model, weight_path):
    ...
    try:
        ...
        state_dict = {k: v for k, v in state_dict.items() if not (k.startswith("sr_detector") or k == "lmbd")}
...
def load_model(model, model_path, optimizer=None, resume=False, 
    ...
    logging.info(f"Before initializing optimizer: Allocated GPU memory: {(torch.cuda.memory_allocated(0) / 2**30):2.2f}GB")
    ...
    if optimizer is not None and resume:
        if 'optimizer' in checkpoint:
            ...
            print('Resumed initializing optimizer with start lr', start_lr)
        else:
            print('No optimizer parameters in checkpoint.')
    logging.info(f"After optimizer: Allocated GPU memory: {(torch.cuda.memory_allocated(0) / 2**30):2.2f}GB")
    return model, optimizer, start_epoch
