...
def train(cfg, model, critetion, optimizer, epoch, data_loader, logger, writer, devices, trainIters, metrics_base='combine'):
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        if i % 5 == 0:
            params = {}
            logger.info(f"Allocated GPU memory: {(torch.cuda.memory_allocated(0) / 2**30):2.2f}GB")
            logger.info(f"Reserved GPU memory: {(torch.cuda.memory_reserved(0) / 2**30):2.2f}GB")
...
def train_simple(cfg, model, critetion, optimizer, epoch, data_loader, logger, writer, devices, trainIters, metrics_base='combine'):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        # logger.info(outputs)
        labels = labels.cuda().to(non_blocking=True, dtype=torch.float64)
        if isinstance(critetion, torch.nn.CrossEntropyLoss):
            labels = labels.to(int).squeeze()
        ...
        if len(outputs.shape) == 2:
            outputs = outputs[:, -1]
        acc_ = ((outputs > 0) == labels).to(torch.float).mean()
...
def validate_simple(cfg, model, critetion, epoch, data_loader, logger, writer, devices, valIters, metrics_base='combine'):
    calculate_acc = get_acc_mesure_func(metrics_base)
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
